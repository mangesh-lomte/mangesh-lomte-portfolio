...
const Contact = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const { firstName, lastName, email, message, permission } = formData;
    ...
    if (
      ...
      console.warn("Validation failed. Required field check:", {
        firstName: Boolean(firstName.trim()),
        lastName: Boolean(lastName.trim()),
        validEmail: Boolean(email.trim()) && validateEmail(email.trim()),
    ...
    if (!serviceId || !templateId || !publicKey) {
      console.error("Credentials missing from environment variables");
      console.log("=== CONTACT DEBUG END ===");
      setStatus('error');
      setTimeout(() => setStatus('idle'), 4000);
      return;
    ...
    console.log("BEFORE EMAILJS REQUEST");
    setStatus('sending');

    const timeoutPromise = new Promise((_, reject) =>
      setTimeout(() => reject(new Error("EMAILJS_REQUEST_TIMEOUT_AFTER_15_SECONDS")), 15000)
    );
    ...
    try {
      ...
      console.log("AFTER EMAILJS REQUEST");
      console.log("EMAILJS RESPONSE:", response);
      console.log("EMAILJS STATUS:", response?.status);
      console.log("EMAILJS TEXT:", response?.text);
      ...
      console.error("EMAILJS REQUEST FAILED OR TIMED OUT");
    ...
    console.log("AFTER EMAILJS DEBUG");
  };
    ...
    <section ref={ref} id="contact" className="bg-[#0a0a0a] w-full min-h-screen relative overflow-hidden flex items-end pt-32 pb-0 md:pb-0 border-t border-gray-900">
      ...
      <div className="relative z-10 w-full flex justify-end items-end">
        ...
        >
          ...
          <form ref={form} onSubmit={handleSubmit} noValidate className="flex flex-col gap-12 md:gap-16 w-full">
            {/* Hidden fields to map to EmailJS template variables */}
            <input
              type="hidden"
              name="first_name"
                ...
                <div className="flex flex-col sm:flex-row sm:justify-between sm:items-end gap-6">
                  <p className="max-w-[250px] leading-relaxed">
                    {personalInfo.emails.primary ? (
                      ...
                      <>Send your inquiry using the form above.</>
                    )}
                  </p>
