...
const ProjectCard = ({ project, aosDelay, onOpenCaseStudy }) => (
  <div 
    ...
    className={`relative rounded-2xl p-[1px] group transition-all duration-500 cursor-pointer ${
      project.isFlagship 
        ? 'bg-gradient-to-br from-red-500/50 via-white/10 to-red-500/30 hover:from-red-500 hover:via-red-400/30 hover:to-red-500/60 shadow-[0_10px_30px_rgba(255,42,42,0.1)]' 
  ...
  >
    <div className={`rounded-2xl p-6 md:p-8 h-full backdrop-blur-md transition-all duration-500 ${
      ...
      <div className="flex justify-between items-center mb-4 flex-wrap gap-2">
        ...
        <span className="text-[11px] font-mono font-bold text-red-400 group-hover:text-white transition-colors flex items-center gap-1">
      ...
      <div className="flex items-baseline gap-4 mb-3">
        ...
        <div>
          <h3 className="text-2xl md:text-3xl font-black text-white tracking-tight group-hover:text-red-400 transition-colors">
            {project.title}
          </h3>
          {project.institution && (
            <p className="text-xs font-mono text-red-300 font-bold mt-1">
              📍 {project.institution}
            </p>
          )}
          {project.course && (
      ...
      <div className="flex flex-wrap gap-3" onClick={(e) => e.stopPropagation()}>
        ...
        <button
          onClick={() => onOpenCaseStudy(project)}
          className="flex items-center gap-2 px-5 py-2 rounded-full bg-[#ff2a2a] text-white text-sm font-bold hover:bg-red-600 hover:shadow-[0_0_20px_rgba(255,42,42,0.4)] transition-all duration-300"
        >
          View Case Study
          <ExternalLinkIcon />
        </button>
      ...
      <div className="max-w-6xl mx-auto relative z-10">
        ...
        <div data-aos="fade-up" className="mb-16 md:mb-20">
          ...
          <p className="text-white/50 text-base md:text-lg max-w-lg font-medium leading-relaxed">
            Click any project to explore detailed engineering case studies, system architecture diagrams, user roles, and challenges solved.
          </p>
        </div>
